//Demonstration of Runge-Kutta integration routine rk1() (initial version by N. Thomas 2012)
//Integrates the differential equation for radioactive decay (modification by Hoi Wong (Ocean))
...
//k will not be declared as it will be changed half-way through the program in 2.3.

//Function definition for deriv().
inline double deriv( double t , double v ) 
{
	return (g- ((t_25 == true) ? 0.003 : 0.003) * v*v);//this was prepared for exercise 2.3, but by changing the value back to 0.003 this can work for 2.2 also.
}
	...
	for(int i=1;i<=nsteps;i++)
		{
			...
			if(t2>=25)//check if parachut should have opened yet.
			{
				t_25= true;
			}
			rk1(v,t1,t2);	//Calls rk1() to integrate deriv() from t1 to t2
			
			double k =((t_25 == true) ?  0.003 : 0.003); //locally declare k;
			v_theo=sqrt(g/k)*tanh(t2*sqrt(g*k)); 	//Analytical solution
				
  			rk1out<<t2<<"\t"<<v<<"\t"<<v_theo<<endl;	//Saves results to diskcout
